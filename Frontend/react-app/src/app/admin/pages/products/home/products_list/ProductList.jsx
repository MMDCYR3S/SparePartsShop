  ...
  <div className="bg-red-50 border-l-4 border-red-500 p-4 rounded-r-xl mb-6 animate-fade-in">
    <div className="flex items-start">
      ...
      <div className="mr-3 w-full">
        <h3 className="text-sm font-bold text-red-800">خطا در بارگذاری اطلاعات</h3>
...
const ProductList = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (list.length > 0 && currentPage > Math.ceil(list.length / itemsPerPage)) {
        setCurrentPage(1);
      }
      ...
      setError({
        userMessage: "ارتباط با سرور برقرار نشد. لطفاً اتصال اینترنت را بررسی کنید یا با پشتیبانی تماس بگیرید.",
  ...
  const handleDelete = async (id) => {
    if (!window.confirm("آیا از حذف این محصول اطمینان دارید؟ این عملیات غیرقابل بازگشت است.")) return;
  ...
  const handleBulkDelete = async () => {
    if (!window.confirm(`آیا از حذف ${selectedIds.size} محصول انتخاب شده اطمینان دارید؟`)) return;
  ...
  const Th = ({ field, label, sortable = true }) => (
    <th 
      className={`px-6 py-4 text-right text-xs font-bold text-gray-500 uppercase tracking-wider ${sortable ? 'cursor-pointer hover:bg-gray-50 group' : ''}`}
      onClick={() => sortable && handleSort(field)}
    >
      <div className="flex items-center gap-2">
        {label}
        {sortable && (
          <ArrowsUpDownIcon className={`w-4 h-4 transition-colors ${ordering.includes(field) ? 'text-accent' : 'text-gray-300 group-hover:text-gray-400'}`} />
        )}
      </div>
    </th>
  );
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">مدیریت انبار و لیست قطعات یدکی</p>
        </div>
        <button 
      ...
      <div className="bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden">
        ...
        {loading ? (
          <div className="h-96 flex items-center justify-center flex-col gap-4">
            <LoadingSpinner />
            <p className="text-gray-400 text-sm animate-pulse">در حال دریافت اطلاعات محصولات...</p>
          </div>
        ) : (
              ...
              <thead className="bg-gray-50 border-b border-gray-100">
                <tr>
                  <th className="px-6 py-4 w-10">
                    <input 
                      type="checkbox" 
                      checked={currentItems.length > 0 && selectedIds.size === products.length}
                      onChange={handleSelectAll}
                      className="w-4 h-4 text-accent bg-gray-100 border-gray-300 rounded focus:ring-accent cursor-pointer"
                    />
                  </th>
                  <th className="px-6 py-4 text-xs font-bold text-gray-500 uppercase">تصویر</th>
                  ...
                  <tr>
                    <td colSpan="8" className="px-6 py-12 text-center">
                      <div className="flex flex-col items-center justify-center gap-3">
                        ...
                        <p className="text-gray-500 font-medium">هیچ محصولی یافت نشد.</p>
                        {search && <button onClick={() => setSearch('')} className="text-accent hover:underline text-sm">پاک کردن جستجو</button>}
                      </div>
                    </td>
                  </tr>
                ) : (
                  currentItems.map((product) => (
                    <tr key={product.id} className={`hover:bg-blue-50/30 transition-colors group ${!product.is_active ? 'bg-gray-50/50 grayscale-[0.5]' : ''}`}>
                      ...
                      <td className="px-6 py-4">
                        <div className="h-12 w-12 rounded-lg border border-gray-200 bg-white p-1 overflow-hidden">
                           {product.images && product.images.length > 0 ? (
                             <img src={product.images.find(img => img.is_main)?.image_url || product.images[0].image_url} alt={product.name} className="w-full h-full object-cover rounded-md" />
                           ) : (
                             <div className="w-full h-full bg-gray-100 flex items-center justify-center text-xs text-gray-400">بدون عکس</div>
                           )}
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        <div className="flex flex-col">
                          ...
                          <span className="text-xs text-gray-400 font-mono mt-1 bg-gray-100 px-2 py-0.5 rounded w-fit">{product.part_code || '---'}</span>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        <span className="text-sm text-gray-600">{product.category_name || 'بدون دسته'}</span>
                        {product.brand && <span className="block text-xs text-gray-400 mt-0.5">{product.brand}</span>}
                      </td>
                      <td className="px-6 py-4">
                        <span className="font-mono font-medium text-gray-700">
                          {product.price ? parseInt(product.price).toLocaleString() : '0'}
                        </span>
                      </td>
                      <td className="px-6 py-4">
                         <span className={`px-2.5 py-1 rounded-full text-xs font-bold ${product.stock_quantity > 5 ? 'bg-emerald-100 text-emerald-700' : product.stock_quantity > 0 ? 'bg-amber-100 text-amber-700' : 'bg-red-100 text-red-700'}`}>
                      ...
                      <td className="px-6 py-4">
                        <button 
                          ...
                          className={`flex items-center gap-1 px-3 py-1 rounded-lg text-xs font-medium transition-colors ${product.is_active ? 'bg-blue-50 text-blue-600 hover:bg-blue-100' : 'bg-gray-100 text-gray-500 hover:bg-gray-200'}`}
                        >
                          {product.is_active ? (
                      ...
                      <td className="px-6 py-4 text-center">
                         <div className="flex items-center justify-center gap-2 opacity-40 group-hover:opacity-100 transition-opacity">
                            <button onClick={() => navigate(`/admin/products/view/${product.id}`)} className="p-2 hover:bg-gray-100 rounded-lg text-gray-600" title="مشاهده جزئیات">
                              <EyeIcon className="w-5 h-5" />
                            </button>
                            <button onClick={() => navigate(`/admin/products/edit/${product.id}`)} className="p-2 hover:bg-blue-50 rounded-lg text-blue-600" title="ویرایش">
                              <PencilSquareIcon className="w-5 h-5" />
                            </button>
                            <button onClick={() => handleDelete(product.id)} className="p-2 hover:bg-red-50 rounded-lg text-red-600" title="حذف">
                              <TrashIcon className="w-5 h-5" />
                            </button>
                         </div>
                      </td>
        ...
        {!loading && products.length > 0 && (
          <div className="px-6 py-4 border-t border-gray-100 bg-gray-50 flex items-center justify-between">
             <span className="text-xs text-gray-500">
               نمایش {indexOfFirstItem + 1} تا {Math.min(indexOfLastItem, products.length)} از {products.length} محصول
             </span>
             <div className="flex gap-1">
               ...
               {Array.from({ length: totalPages }, (_, i) => (
                 <button
                    ...
                    className={`w-8 h-8 rounded border text-sm flex items-center justify-center transition-colors ${currentPage === i + 1 ? 'bg-primary text-white border-primary' : 'bg-white border-gray-300 hover:bg-gray-50'}`}
                 >
                   {i + 1}
                 </button>
               ))}
